...
>>>>>>> c4a66415263fa8c7b18cf50f4aa83a12e3a60a47
      {payload.map((entry: any, index: number) => {
        ...
        return (
          <div key={`item-${index}`} className="flex items-center gap-3 cursor-pointer group select-none" onClick={() => onClick(entry.dataKey)}>
            <div className="w-5 h-1.5 rounded-full transition-all" style={{ backgroundColor: isVisible ? entry.color : '#e2e8f0', opacity: isVisible ? 1 : 0.3 }} />
            <span className={`text-[13px] font-black uppercase tracking-tight transition-all ${isVisible ? '' : 'text-slate-300 line-through'} group-hover:brightness-75`} style={{ color: isVisible ? entry.color : undefined }}>{displayName}</span>
          </div>
        );
      })}
    </div>
...
const useContinuousPress = (callback: () => void, onStart?: () => void) => {
  ...
  const start = useCallback((e: React.MouseEvent | React.TouchEvent) => {
    ...
    timeoutRef.current = window.setTimeout(() => {
      intervalRef.current = window.setInterval(() => { callbackRef.current(); }, 200);
    }, 500);
  }, [stop, onStart]);
  useEffect(() => { return () => stop(); }, [stop]);
  return { onMouseDown: start, onTouchStart: start, onMouseLeave: stop };
};
...
const LogChart: React.FC<LogChartProps> = memo(({ 
  data, highlightedTime, leftOffset, leftScale, rightOffset, rightScale, onSettingsChange, ticks 
}) => {
  const [visibility, setVisibility] = useState<ChartVisibility>({
    airTemp: true, airHtLvl: true, airHtPt100: true, humidity: true, humiHtLvl: true,
    humiHtPt100: true, warmHtLvl: true, warmHtPt100: true, oxygen: true, skin1Temp: true,
    skin2Temp: true, waterLvl: true,
  });

  const [focusContext, setFocusContext] = useState<{ channel: 'left' | 'right', param: 'scale' | 'offset' }>({ channel: 'left', param: 'scale' });
  
  // [추가] 클릭된 상태 관리를 위한 state
  const [isZeroRefVisible, setIsZeroRefVisible] = useState(false);

  const p = (val: number) => (typeof val !== 'number' || isNaN(val)) ? 0 : Math.round(val * 100) / 100;

  const leftDomain = useMemo<[number, number]>(() => [p(leftOffset), p(leftScale + leftOffset)], [leftOffset, leftScale]);
  const rightDomain = useMemo<[number, number]>(() => [p(rightOffset), p(rightScale + rightOffset)], [rightOffset, rightScale]);

  const toggleVisibility = useCallback((dataKey: string) => setVisibility(prev => ({ ...prev, [dataKey]: !prev[dataKey] })), []);
  const toggleAllVisibility = useCallback((show: boolean) => {
    setVisibility(prev => {
      const next = { ...prev };
      Object.keys(next).forEach(key => { next[key as keyof ChartVisibility] = show; });
      return next;
    });
  }, []);
...
>>>>>>> c4a66415263fa8c7b18cf50f4aa83a12e3a60a47
  const adjustValue = useCallback((channel: 'left' | 'right', param: 'scale' | 'offset', direction: 1 | -1) => {
    const step = STEP;
    if (channel === 'left') {
      if (param === 'scale') onSettingsChange({ leftScale: p(Math.max(5, leftScale + (direction * step))) });
      else onSettingsChange({ leftOffset: p(leftOffset + (direction * step)) });
    } else {
      if (param === 'scale') onSettingsChange({ rightScale: p(Math.max(5, rightScale + (direction * step))) });
      else onSettingsChange({ rightOffset: p(rightOffset + (direction * step)) });
    }
  }, [leftOffset, leftScale, rightOffset, rightScale, onSettingsChange]);

  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'ArrowUp') { e.preventDefault(); adjustValue(focusContext.channel, focusContext.param, 1); }
      else if (e.key === 'ArrowDown') { e.preventDefault(); adjustValue(focusContext.channel, focusContext.param, -1); }
    };
  ...
  // 1. 상태 선언 (데이터 값이 아닌 픽셀 좌표를 임시로 쓰거나 계산된 값을 저장)
const [clickedYValue, setClickedYValue] = useState<number | null>(null);
...
const handleChartClick = useCallback((nextState: any) => {
  // 클릭한 지점에 데이터가 있는지 확인 (Recharts의 클릭 이벤트 객체 구조)
  if (nextState && (nextState.activePayload || nextState.activeCoordinate)) {
    setIsZeroRefVisible(prev => !prev);

  const { chartY } = nextState;
  
  // 2. 차트의 실제 그리기 영역(픽셀) 계산
  // LineChart에 설정한 margin { top: 40, bottom: 15 }을 기준으로 합니다.
  // 컨테이너 전체 높이를 구하기 위해 이벤트 타겟의 높이를 참조하거나 상수를 사용합니다.
  const chartHeight = 500; // 사용하는 ResponsiveContainer의 대략적인 높이 (또는 고정 높이)
  const marginTop = 40;
  const marginBottom = 15;
  const graphInnerHeight = chartHeight - marginTop - marginBottom;

  // 3. 마우스 위치가 차트 가동 범위 안에 있는지 확인
  if (chartY >= marginTop && chartY <= chartHeight - marginBottom) {
    // 4. 픽셀 위치를 데이터 값으로 역산
    // 공식: (1 - (현재Y - 시작Y) / 전체높이) * 스케일 + 오프셋
    const ratio = 1 - ((chartY - marginTop) / graphInnerHeight);
    const calculatedValue = p(leftOffset + (ratio * leftScale));

    console.log("Clicked Value:", calculatedValue); // 디버깅용
    setClickedYValue(prev => (prev === calculatedValue ? null : calculatedValue));
  }
  
  setClickedYValue(getRandomValue())
    console.log("Chart Clicked! Zero Ref:", !isZeroRefVisible , clickedYValue); // 디버깅용
  }
}, [isZeroRefVisible]);
  ...
  const legendPayload = useMemo(() => legendItems.map(item => ({ dataKey: item.dataKey, value: item.name, type: 'line' as const, color: item.color })), [legendItems]);
  const formatXAxis = (ts: number) => ts ? new Date(ts).toLocaleTimeString('en-GB', { hour12: false }) : '';
  ...
  const OscBtnControl = ({ label, value, channel, param, colorClass, icon }: any) => {
    ...
    const onAction = (dir: 1 | -1) => adjustValue(channel, param, dir);
    const setFocus = () => setFocusContext({ channel, param });
...
>>>>>>> c4a66415263fa8c7b18cf50f4aa83a12e3a60a47
        ...
        <button {...pressDown} className="w-12 h-12 flex items-center justify-center bg-white border border-gray-200 rounded-b-2xl hover:bg-gray-50 text-gray-600 transition-colors shadow-sm active:bg-gray-100 outline-none select-none touch-none"><svg className="w-6 h-6 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M19 9l-7 7-7-7" /></svg></button>
      </div>
    );
  };
  ...
  return (
    <div className="w-full h-full bg-white flex flex-col p-2 overflow-hidden">
      <div className="flex-1 flex gap-5 min-h-0">
        <div className="flex flex-col items-center justify-center gap-5 px-4 py-8 bg-gray-50/50 rounded-[2.5rem] border border-gray-100 shadow-sm min-w-[110px] no-export">
          ...
          <OscBtnControl label="SCALE" value={leftScale} channel="left" param="scale" colorClass="text-red-600" icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M4 8l8-8 8 8M4 16l8 8 8-8" /></svg>} />
          <OscBtnControl label="OFFSET" value={leftOffset} channel="left" param="offset" colorClass="text-red-500" icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" /></svg>} />
...
>>>>>>> c4a66415263fa8c7b18cf50f4aa83a12e3a60a47
              ...
              {highlightedTime && (
                <ReferenceLine yAxisId="left" x={highlightedTime} stroke="#ef4444" strokeWidth={3} strokeDasharray="8 4" label={{ position: 'top', value: 'ALARM', fill: '#ef4444', fontSize: 14, fontWeight: '900' }} />
              )}
...
>>>>>>> c4a66415263fa8c7b18cf50f4aa83a12e3a60a47
        ...
        <div className="flex flex-col items-center justify-center gap-5 px-4 py-8 bg-gray-50/50 rounded-[2.5rem] border border-gray-100 shadow-sm min-w-[110px] no-export">
          ...
          <OscBtnControl label="SCALE" value={rightScale} channel="right" param="scale" colorClass="text-blue-600" icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M4 8l8-8 8 8M4 16l8 8 8-8" /></svg>} />
          <OscBtnControl label="OFFSET" value={rightOffset} channel="right" param="offset" colorClass="text-blue-500" icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" /></svg>} />
          <button onClick={() => onSettingsChange({ rightOffset: 0, rightScale: 120 })} className="mt-2 w-20 py-2 bg-gray-200 hover:bg-blue-500 hover:text-white text-gray-500 rounded-xl text-[10px] font-black uppercase transition-all shadow-sm active:scale-95">RESET</button>
        </div>
      </div>
    </div>
  );
