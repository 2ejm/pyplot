...
const SidebarFileList: React.FC<SidebarFileListProps> = ({ 
  ...
  // Only show log files in the sidebar
  const items = isDrive 
    ...
    <div className="bg-white border border-gray-200 p-3 rounded-xl shadow-sm w-full flex flex-col gap-2 transition-all">
      <div className="flex justify-between items-center border-b border-indigo-50 pb-1.5">
        <h3 className="text-indigo-600 font-black text-[10px] uppercase tracking-widest">
          [ {isDrive ? 'CLOUD DIRECTORY' : (isLocal ? 'LOCAL FILES' : 'ACTIVE SOURCE')} ]
        </h3>
        <span className="text-[8px] font-bold text-gray-400">{items.length} LOGS</span>
      </div>
      ...
      <div className="max-h-[240px] overflow-y-auto space-y-1 pr-1 custom-scrollbar">
        {items.length > 0 ? (
          items.map((item, idx) => {
            ...
            return (
              <button
                key={isDrive ? (item as DriveFile).id : idx}
      ...
      {sourceType && (
        <div className="pt-2 mt-1 border-t border-slate-50 flex justify-center">
          <p className="text-[8px] font-bold text-slate-400 uppercase tracking-tighter italic">
            Select a file to sync telemetry
          </p>
        </div>
      )}
    </div>
